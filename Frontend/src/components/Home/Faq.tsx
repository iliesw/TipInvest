    ...
    <section className="py-24">
      <div className="mx-auto max-w-7xl">
        <div className="mb-16 text-center">
          <h6 className="text-lime-600 font-semibold pb-6">FAQs</h6>
        ...
        <div>
          {faqs.map((faq, index) => (
            <div
              key={index}
              className={`py-8 px-6 border-b border-gray-200 rounded-2xl transition-all duration-500 cursor-pointer hover:bg-lime-50 ${
                openIndex === index ? "bg-lime-50" : ""
              }`}
              onClick={() => toggleFAQ(index)}
            >
              <button className="flex items-center justify-between w-full text-left text-gray-900 leading-8 font-medium hover:text-lime-600">
                <h5>{faq.question}</h5>
                <svg
